...
- **The local fleet-private string guard silently does not run for `cd <path> && git commit ...` — the dominant Claude Code commit shape.** Measured 2026-08-20. The `PreToolUse` hook in `~/.claude/settings.json` carries `"if": "Bash(git commit*)"`, which is a **prefix** match on the whole command string. A bare `git commit` matches and the guard fires (verified: blocked, exit 2). A command beginning `cd ~/Projects/ops-brain && git add … && git commit …` does **not** match, so `pre-commit-check.sh` is never invoked at all. The irony is that the script contains careful logic to extract the repo from exactly that `cd`-prefixed form — logic that is dead code in practice, because the settings-level `if` filters that shape out before the script ever sees it. Running the script directly against all three invocation shapes blocks all three, so testing the script proves nothing about whether the hook *ran*. This is how a deploy hostname reached `TODO.md` on PR #84 and got caught only by CI. **A local commit succeeding is not evidence the guard ran.** Grep the branch diff yourself before pushing to this public repo.
- **The CI guard diffs merge-base→head (two-dot), not per-commit.** A fleet-private string introduced by an earlier commit on the branch is cleared by a *fix-forward* commit that removes it — the cumulative diff no longer contains the added line, so no rebase, amend, or history rewrite is needed. Verify locally before pushing, reusing the workflow's own pattern rather than copying it here (copying the literal hostnames into this file would itself trip the guard — a real constraint when documenting it):
  ```sh
  PAT=$(grep -oP "(?<=PATTERN=').*(?=')" .github/workflows/secret-scan.yml)
  git diff "$(git merge-base origin/main HEAD)" HEAD \
    -- ':(exclude).github/workflows/secret-scan.yml' \
    | grep -E '^\+[^+]' | grep -nE "$PAT"
  ```
  Historical hits already on `main` — e.g. the hostname list in `migrations/20260426000002_normalize_handoff_machine_names.sql` — do not fail the guard, since they are not additions in the branch diff.
...
- **PowerShell 7.2 can corrupt inline JSON passed to native executables.** Its legacy native-argument marshalling may strip embedded quotes from values such as Claude Code's `--mcp-config` JSON. The Windows live launcher deliberately writes the generated MCP configuration to a securely-created temporary file, passes only that path, and deletes it in `finally`; do not “simplify” it back to an inline JSON argument. The Windows CI test runs a fake `claude.cmd` and verifies the received config parses plus the temporary file is removed.
