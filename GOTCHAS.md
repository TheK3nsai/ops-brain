...
- **PowerShell 7.2 can corrupt inline JSON passed to native executables.** Its legacy native-argument marshalling may strip embedded quotes from values such as Claude Code's `--mcp-config` JSON. The Windows live launcher deliberately writes the generated MCP configuration to a securely-created temporary file, passes only that path, and deletes it in `finally`; do not “simplify” it back to an inline JSON argument. The Windows CI test compiles a fake `claude.exe` and verifies the received config parses plus the temporary file is removed.
