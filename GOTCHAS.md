...
- **Identity enforcement fails OPEN if the `http` crate ever splits versions across `axum` and `rmcp`.** Per-agent token identity (`CallerClass`) travels: `bearer_auth` inserts it into the axum request extensions → rmcp injects that request's `http::request::Parts` (extensions included) into the tool-call context → handlers read it via `helpers::bound_agent` doing `ext.get::<axum::http::request::Parts>()`. That downcast is by `TypeId`, so it only fires when `axum::http::request::Parts` and the `Parts` rmcp inserts are **the same type** — i.e. the crate graph resolves `http` to a single version (today: one `http` v1.x for both). If a future `axum`/`rmcp` bump pulls in a second `http` major, `get::<Parts>()` returns `None`, `bound_agent` yields `None`, and every write is treated as an unbound (main-bearer) caller — the `from_agent`/`author` binding silently stops enforcing. It fails *open*, not closed, and no test catches it (the handler enforcement tests pass `bound` directly; the middleware test only proves insertion into axum's extensions, not survival into the rmcp context). After any `axum`/`rmcp`/`http` dep bump: `cargo tree -i http` must show one version, and smoke-test a real per-agent-token `create_handoff` with a mismatched `from_agent` → it must still be rejected.
