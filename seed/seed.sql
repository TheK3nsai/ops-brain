...
-- Knowledge entries
INSERT INTO knowledge (id, title, content, category, tags, client_id, cross_client_safe) VALUES
    ('019594a0-0006-7000-8000-000000000001', 'Splashtop Remote Access', 'Eduardo uses Splashtop Business for remote access to all client machines. Runs via Wine on Gentoo Linux. Desktop entry at ~/.local/share/applications/splashtop-business.desktop.', 'remote-access', '{splashtop,wine,remote}', NULL, false),
    ('019594a0-0006-7000-8000-000000000002', 'HSR Network Architecture', 'HSR has two sites connected via site-to-site VPN. Main office is 10.10.1.0/24, Renacer is 10.10.2.0/24. Each site has its own domain controller. All servers are physical with Hyper-V role.', 'networking', '{hsr,network,vpn}', '019594a0-0001-7000-8000-000000000001', false),
    ('019594a0-0006-7000-8000-000000000003', 'kensai.cloud Stack', 'Rocky Linux 10.1 VPS running Docker. Caddy reverse proxy (replaced Traefik). cloudflared tunnel for Cloudflare connectivity. Services: Nextcloud, Zammad, InvoicePlane, Collabora, Homer. PostgreSQL 18 and MariaDB 11.8 for databases. SSH via ssh.kensai.cloud:22022.', 'infrastructure', '{cloud,docker,caddy}', NULL, false),
    ('019594a0-0006-7000-8000-000000000004', 'WARP VPN for HSR Access', 'Cloudflare WARP / Zero Trust provides VPN tunnel to HSR servers. All 6 hospice servers accessible via direct SSH through WARP tunnel. Configured on stealth (Gentoo laptop).', 'vpn', '{warp,cloudflare,hsr,vpn}', '019594a0-0001-7000-8000-000000000001', false),
    ('019594a0-0006-7000-8000-000000000005', 'Eduardo Role and Responsibilities', 'Solo IT Director (MSP/consultant). Manages two clients: HSR hospice (~300 employees, ~400 patients, 2 locations) and a CPA firm (4 employees). Uses CC across 3 machines: stealth (Gentoo laptop), HSR servers, kensai.cloud VPS.', 'operations', '{role,msp,management}', NULL, false)
ON CONFLICT DO NOTHING;

-- Sample runbook (HSR-scoped — Veeam is on HV-FS0 which is HSR infrastructure)
INSERT INTO runbooks (id, title, slug, category, content, version, tags, estimated_minutes, requires_reboot, client_id, cross_client_safe) VALUES
    ('019594a0-0007-7000-8000-000000000001', 'Veeam Backup Conflict Resolution', 'veeam-backup-conflict', 'backup', E'# Veeam Backup Conflict Resolution\n\n## Symptoms\n- SMB timeouts during backup window\n- File share access slow or unresponsive\n- Veeam job overlapping with other operations\n\n## Steps\n1. Check Veeam job schedule in Veeam console\n2. Verify no overlapping backup jobs\n3. Check HV-FS0 resource usage during backup window\n4. Adjust backup schedule if conflicts found\n5. Consider staggering backup jobs across servers\n\n## Prevention\n- Keep backup jobs at least 2 hours apart\n- Monitor HV-FS0 disk I/O during backups\n- Set up alerts for backup job failures', 1, '{veeam,backup,smb,troubleshooting}', 30, false, '019594a0-0001-7000-8000-000000000001', false)
ON CONFLICT (slug) DO UPDATE SET content = EXCLUDED.content, version = runbooks.version + 1, client_id = EXCLUDED.client_id, cross_client_safe = EXCLUDED.cross_client_safe;
