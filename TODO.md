...
- **~~The guard publishes its own denylist~~ — CLOSED 2026-08-24 (CC-HSR ruling `01a03571`).** The workflow no longer contains a plaintext pattern; the denylist ships as salted SHA-256 digests and the scan reads the whole tracked tree. Mechanism note: the ruling specified a **repository secret**, and that was rejected on measurement — this repo is public, and `pull_request` runs from forks receive no secrets, so a secret-sourced denylist is either blind on fork PRs or red on every one of them. Hashes behave identically for forks and need no provisioning. Same goal, strictly fewer failure modes. Details in `GOTCHAS.md`.
- **The local (dotfiles) and CI guards have diverged — decide whether to converge them.** `dotfiles/claude/.claude/hooks/pre-commit-check.sh` is still a case-*sensitive* plaintext regex over `git diff --cached`; CI is now hash-based, case-insensitive, and full-tree. dotfiles is private so the plaintext there is not itself an exposure, but the local hook now misses classes CI catches — notably every lower-case variant, which is the shape the migration actually uses. Cheapest convergence: have the hook shell out to `.github/scripts/fleet_string_guard.py` when the repo is ops-brain, and keep its own pattern only as the fallback for other repos. Not done here to keep this PR to one repo.
- **Migration remediation is blocked on a production DB mutation, not on code — CC-HSR ruling item 3 cannot land as written.** `migrations/20260426000002_normalize_handoff_machine_names.sql` carries **16** occurrences of the three client-private classes (3 upper-case in the header comment, 13 lower-case and load-bearing in the `CASE`/`WHERE` clauses). Note the count has been recorded three different ways in this thread — `6` here, `9` in the ruling — both wrong; 16 is measured, case-insensitively, and reproducible with `.github/scripts/fleet_string_guard.py` (the waiver warning prints the occurrence count; `--no-allowlist` shows each line). Editing the file at all trips sqlx's applied-migration checksum check (`sqlx-core` 0.8.6 `migrator.rs:176`, no ignore option) and stops the deployed instance booting until `_sqlx_migrations` is updated in lockstep — a coordinated deploy + DB write, CC-Cloud's lane, Eduardo's call. The guard allowlists the path with that exit condition recorded. **Also worth deciding at the same time:** per ruling item 4 these strings are treated as *disclosed* (public `main` since 2026-04-26), so scrubbing the working tree changes what a new clone carries but recalls nothing. The honest options are (a) squash the migration set to a baseline, which drops the file without a checksum conflict on a fresh DB, or (b) accept it and stop paying attention to this one file. A comment-only scrub costs a coordinated deploy to remove 3 of 16 occurrences and is the worst of both.
