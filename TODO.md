...
- **v5.2.0 shipped the Windows Claude adapter log fix untested on a real Windows host.** CI's `Windows ops-brain clients` job is green on every PR, but the lane has never been run live on either Windows client host. Eduardo chose to cut the release first and test after (2026-08-26). Windows is where the original defect bit hardest — the Claude adapter wrote no log at all there, so a channel that failed to bind was invisible — and that fix is the one still unexercised. **Do not call the Windows pair live until an attended run from the v5.2.0 client bundle (`c8ec0d5`) passes.** Treat that run as the validation gate, not an already-approved rollout; record the independently deployed server revision alongside the client revision, and do not substitute the server's newer checkout for the released client baseline. A green CI job remains insufficient evidence for this lane.
