...
- **v5.2.1 is the published replacement for the failed v5.2.0 Windows gate; do not call the Windows pair live yet.** The attested release ZIP contains both the cross-platform dependency-manifest fix and #103's process-global Codex thread selection fix. CI now models both observed ZIP symlink materializations, but a green CI job remains insufficient evidence for this lane. Production deployment is accepted and queued to CC-Cloud on handoff `01a05da2-c1c4-7ae0-9058-a8a3773d9b84`; after its readiness receipt, repeat the complete attended pair gate from the exact v5.2.1 ZIP and record client and server revisions.
- **v5.2.1 also addresses four non-blocking findings from the failed gate:** graceful Claude shutdown now records a disconnect; the Codex PowerShell launcher resolves the npm package's vendored native executable; Claude overlay cleanup handles both junctions and runtime-created directories; and the runbook distinguishes Bash flags from PowerShell's `-Mode Status|DryRun`. These remain unvalidated on a real Windows host until the exact v5.2.1 bundle passes the attended pair gate.
