...
- **Codex thread selection disqualifies on *any* RPC rejection, not just a missing rollout (PR #103, 2026-08-26).** `#resolveThread` probes every process-wide loaded thread with `thread/resume` and `continue`s past any `RpcError`. That is what unblocked the 0.150.0 gate — a clean TUI launch exposes a second, rollout-less in-memory thread — but it also means that with two genuinely persisted threads loaded, a transient or unrelated rejection on one makes the adapter latch the *other*, which may be a different session's thread. The previous count-based code stayed offline there. Narrowing the skip to a no-rollout-shaped rejection buys fail-closed behaviour at the cost of matching Codex's error wording, which is exactly the churn this fix is reacting to — so it was deliberately not done in #103. Revisit when the Codex error shape is pinned (a stable code, not a message), or if a wrong-thread injection is ever observed. The README's "confirms have no persisted rollout" wording runs slightly ahead of the code and should be corrected in the same change.
- **Integrate live mode into each host's main Claude/Codex launcher.** The explicit-launcher fleet gate is clean on both Linux and Windows as of 2026-09-01, so adapter acceptance no longer blocks this work. The current explicit `ops-brain-claude` / `ops-brain-codex` entry points remain the safe rollout boundary until the integrated path is itself tested. Keep the adapter owned by the foreground session; preserve ordinary arguments, TTY ownership, signals, and exit status; never turn it into a service or wake dependency. Startup must distinguish connected, degraded/fallback, and intentionally ordinary/no-live modes visibly. If live was requested and preflight fails, require affirmative operator choice before continuing ordinary or exit nonzero—automatic fallback recreates the measured “healthy session, absent lane” defect even with a banner. Do not replace the explicit commands until the main-launcher path passes the same two-identity, rendered-marker, idle, and teardown gate on every host class.

- **The Windows Claude overlay repeats first-run onboarding on every launch (v5.2.1 gate, Claude Code 2.1.257).** The generated `.claude.json` carries only the Channel server, so Claude presents the theme and usage prompts again and does not spawn the adapter until an operator completes them; this delayed an attended gate by about four minutes. Seed only the verified onboarding-complete field in the generated overlay. Do not copy the real `.claude.json`: it can contain unrelated state and MCP credentials, and preserving that isolation is the reason the overlay exists. Add a fixture proving the allowlist carries onboarding state but no existing server definition or secret-shaped value.
...
## Windows live-lane validation (passed 2026-09-01)

- **The CPA Windows pair is certified live on the exact v5.2.1 release bundle.** The attended nine-step gate passed with client `02bd845`, deployed server `5da5456`, PowerShell 7.6.5, Node 24.13.1, Claude Code 2.1.257, and Codex CLI 0.151.0. Both dependency trees verified without `npm ci`; both isolated start/stop checks, the paired identity controls, rendered delivery in both directions, the 190-second idle control, and sequential teardown passed. The release ZIP matched `SHA256SUMS` and its GitHub attestation verified. Sanitized receipt location and the host workspace commit are recorded on handoff `01a05ea8-8559-7760-935b-868ead160f27`.
- **Three Windows fixes are measured closed:** the Codex launcher resolves its vendored native executable, Claude overlay cleanup removes runtime-created directories, and the runbook's PowerShell interface matches the launchers. The graceful Claude disconnect implementation does not emit a terminal record on normal Windows exit because Claude Code kills its MCP child before the adapter receives a graceful signal. This is non-blocking: isolated and paired peer disappearance and owned-process cleanup passed. The runbook now names the platform-specific evidence boundary.
- **The Stealth cold-start residual is closed.** CPA started Codex alone from zero, submitted the initial prompt, observed exactly one `Codex-CPA` peer matching the adapter log, then measured the peer and owned processes disappear on exit. The prerequisite now targets measured Codex CLI 0.151.0; 0.150.0 exact remains unmeasured, not an operational acceptance blocker.
- **`5.0.0` and `5.1.0` were never tagged** — CHANGELOG sections and crate versions exist, but no images or client bundles, so artifact history jumps v4.2.0 → v5.2.0 and continues at v5.2.1. Backfill via the Release workflow's `workflow_dispatch` `tag` input if it's ever wanted. Not urgent; recorded so nobody assumes an artifact exists because the changelog says the version shipped.
