...
- **Codex client acceptance is pinned to 0.149.x until 0.150.0 clears a full attended gate.** `docs/live-fleet-rollout.md` carries the pin. 0.150.0 is *not* known bad after #103 — it is unproven, because the mid-gate upgrade on the Cloud host destroyed the 0.149 `found N` baseline before the thread-count stage was ever reached. Lifting the pin needs one clean 0.150.0 launch plus the complete rendered-delivery gate, with `codex --version` recorded on the receipt. Every host still to roll out should be told which Codex to be on *before* it launches.
...
- **v5.2.0 failed the attended Windows gate on CPA-SRV (2026-08-31). Do not call the Windows pair live.** The Claude half of the release did pass in isolation: its JSON log captured startup and registration, and the logged peer ID matched the server. The pair is blocked by two release-bundle defects that CI did not model. First, the Linux-built `DEPENDENCIES.json` records `node_modules/.bin/node-which` as a symlink while Windows zip extraction materializes the same bytes as a regular file, so the published bundle can never report both dependency trees `ready` on Windows. Second, v5.2.0 predates #103 (`aaa107c`), so Codex CLI 0.151.0's second process-global loaded thread leaves the released adapter waiting forever for exactly one thread. #103 is on `main` but in no release. **Do not cut or approve a replacement bundle until the cross-platform manifest case is fixed and tested against the materialized-file shape; then cut a release containing both that fix and #103 and repeat the attended pair gate from that exact release bundle.** Record client and server revisions in the receipt. A green CI job remains insufficient evidence for this lane.
- **The replacement-release patch also addresses four non-blocking findings from the failed gate:** graceful Claude shutdown now records a disconnect; the Codex PowerShell launcher resolves the npm package's vendored native executable; Claude overlay cleanup handles both junctions and runtime-created directories; and the runbook distinguishes Bash flags from PowerShell's `-Mode Status|DryRun`. These remain unvalidated on a real Windows host until the exact replacement release bundle passes the attended pair gate.
