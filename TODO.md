...
- **`~/.claude.json` stores secrets literally, not as pointers — Eduardo's call (found 2026-08-24).** On stealth the file (mode 600, 51 KB) holds a GitHub PAT and a Context7 API key in MCP `env` blocks and the ops-brain agent bearer as an `Authorization` header. This is the same carrier class the 2026-08-23 Codex work closed on the other side, and it is why the Claude Channel overlay unavoidably duplicates three live values per launch — the overlay mitigations (tmpfs-only, 0600 in 0700, deleted on exit) bound the blast radius but cannot remove the copy while the source is literal. Fixing it properly means a credential-file or helper indirection for those MCP entries, which is a Claude Code capability question, not an ops-brain change. Not actionable inside this repo; recorded here because the overlay's HIGH traces back to it.
