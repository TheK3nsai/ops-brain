...
- **The local (dotfiles) guard is now weaker than the CI guard, in exactly the case that mattered — a real gap, not tidy-up (CC-HSR, `01a035fa`).** `dotfiles/claude/.claude/hooks/pre-commit-check.sh` is still a case-*sensitive* plaintext regex over `git diff --cached`; CI is now hash-based, case-insensitive, and full-tree. dotfiles is private so the plaintext there is not itself an exposure, but the local hook now misses classes CI catches — notably every lower-case variant, which is the shape the migration actually uses. Cheapest convergence: have the hook shell out to `.github/scripts/fleet_string_guard.py` when the repo is ops-brain, and keep its own pattern only as the fallback for other repos. Not done in #93 to keep that PR to one repo. **Framing per CC-HSR's ruling: anyone trusting the pre-commit hook is trusting the version that already proved blind.** dotfiles being private makes its plaintext not-an-exposure; it does not make the divergence harmless.
- **~~Migration remediation~~ — RULED 2026-08-24 (CC-HSR, `01a035fa`): do not touch the migration for this. Not open work; kept as a rider.** `migrations/20260426000002_normalize_handoff_machine_names.sql` carries **16** occurrences of the three client-private classes (3 upper-case in the header comment, 13 lower-case and load-bearing in the `CASE`/`WHERE` clauses) — measured case-insensitively, reproducible with `.github/scripts/fleet_string_guard.py --no-allowlist`. Editing the file at all trips sqlx's applied-migration checksum check (`sqlx-core` 0.8.6 `migrator.rs:176`, no ignore option) and stops the deployed instance booting until `_sqlx_migrations` is updated in lockstep — a coordinated prod deploy plus a DB write to remove 3 of 16 occurrences. Per ruling item 4 these strings are treated as *disclosed* (public `main` since 2026-04-26), so scrubbing `HEAD` changes what a new clone carries and recalls nothing. **Ruling: option (b) — accept it and stop paying attention to this file; option (a) folded in opportunistically.** **Rider:** when the migration set is next squashed to a baseline for unrelated reasons, this file drops with no checksum conflict on a fresh DB and the exposure resolves at zero marginal cost. Do not raise it as its own task. The guard allowlists the path and still warns every run with the occurrence count, so the waiver cannot silently become permanent — CC-HSR endorsed that over a red `main`.
