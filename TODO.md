...
- **v5.1.0 client bundle: two release blockers confirmed in `7c4e534` (2026-08-24).** Found re-reviewing the launcher patch; reproduced against a bundle built from the committed tree, both still open. **(1) `deps-status`/`doctor` are not per-client.** `scripts/ops-brain-client:248` walks *both* adapters (`for (const client of CLIENTS) visit(...)`) when verifying `DEPENDENCIES.json`, so codex-only drift makes `deps-status claude` report `missing-or-invalid` and `doctor claude` exit 1 blaming the untouched claude adapter. This re-breaks the blocker the patch set out to fix, through a different path, in *every* shipped bundle — `build-client-bundle:84` always writes the manifest. The existing test misses it because its fixture has no manifest. **(2) The manifest fails open:** `:222` is `existsSync(m) && !verify(m)`, so deleting `DEPENDENCIES.json` makes a tampered bundle report `ready`. In a bundle context, absent manifest must be `missing-or-invalid`. Fix owner is Codex-Stealth (handoff `01a0352f` / correction `01a03531`); re-run the bundle repro against any fix rather than the fixture.
