...
- **The briefing leads with what needs the operator.** `POST /api/briefing`
  ...
  **waiting on you** (open action handoffs addressed to the operator slug,
  ...
  machine-filed findings are counted, never titled — in production they
  dominated the old list and buried items that had been waiting weeks. An
  optional `operator` field names the slug the first section reads (default
  `Operator`); requests that omit it are unchanged. REST-only, so the MCP
  surface and every agent's token cost are untouched. The JSON keeps its
  existing keys and gains `operator`, `waiting_on_you`, `stuck`, and `counts`.
...
- **The notifier no longer shares fate with the channel it reports on** (#79). A failed send already held the cursor and retried, so nothing was ever lost — but a dead mailer and a quiet bus looked identical from the operator's side, and on 2026-07-28 one revoked app password took the briefing, the security digest, and this notifier down together, making the handoff that reported it undeliverable. `scripts/operator-notify.sh` gained an optional `$OPS_NOTIFY_HEARTBEAT_URL` pinged `up`/`down` with a reason on every real run, so a **machine** whose job is noticing silence — not a human — detects the channel going dark. The escalation path is one already exercised daily by other monitors rather than a second credential that would sit idle and rot; alert thresholds and routing stay in the monitoring system, so there is **no failure counter and no new state** here. Pinging is best-effort by construction: an unreachable monitor logs a line and changes neither exit code nor cursor, because the channel that reports failures must not become a new way to fail. `--dry-run` never pings. Covers a dead *channel*, not a dead *host*.
...
- **Safety-critical paths are now covered by integration tests** (locked against real Postgres + the real axum middleware): the cross-client gate end-to-end through `search_knowledge` (withhold → `acknowledge_cross_client` release → `audit_log` row, cross-client-safe passthrough, provenance stamping), `bearer_auth` classification/scope/path enforcement (main→Full, machine-token 403 on non-machine paths and wrong scope, 401 on garbage/missing bearer, dev-mode Full when no token is configured), the `POST /api/handoff` identity-binding branches (from_agent mismatch → 400, missing/​disallowed to_agent → 400/403, happy path → 201 `origin=machine`), and exact (non-ILIKE) agent-name matching on the list filters. 14 new integration tests.
- **`generate_briefing` dropped its `client_slug` parameter** (MCP tool + REST `POST /api/briefing`). **Minor breaking change for MCP callers** that passed `client_slug` — the field is now ignored/rejected. Briefings were always posted fleet-wide by the cron (`{"type": ...}`); scoping was dead weight. The `briefings.client_id` column is retained for historical rows (new rows insert NULL). The title header drops the per-client name and reads `# Daily/Weekly Operational Briefing` with a `## Handoffs (fleet-wide)` section.
- **Briefing open/pending/accepted counts are now real `SELECT count(*)` totals** via `count_open_handoffs`, not `len()` of a `LIMIT 20` page — the header no longer under-reports on a busy bus. Title lists stay bounded at 20 and are labeled "showing N most recent of M" when they undercount.
- **`generate_briefing_inner` + `build_markdown` moved from `api.rs` into `tools/briefings.rs`**, fixing the layering inversion where the MCP tool called up into the REST module; `api::generate_briefing` is now a thin HTTP wrapper.
- **Merged `list_handoffs` / `list_open_handoffs` into one builder** (`list_handoffs_filtered` + a `StatusFilter { Any, Exact, Open }` enum); the two public signatures are unchanged thin wrappers. The notify-TTL read-time pruning clause now lives in exactly one place.
- **De-duplicated the search/browse arm logic** in `search_knowledge`: the repeated filter→cross-client-gate→compact→provenance→audit shape is now two shared helpers (`process_knowledge_arm`, `process_handoff_arm`) used by single-table search, multi-table search, and browse. Behavior preserved (search audit-logs surfacing attempts; browse still does not).
- **Unscoped knowledge queries are now explicit about the inert safety gate**: `search_knowledge` and `list_knowledge` with no `client_slug` stamp `_client_slug`/`_client_name` provenance on every item (`list_knowledge` previously stamped none) and carry a top-level `_note` explaining that cross-client withholding is not applied without a client scope. `list_knowledge` now returns an object (`{ "knowledge": [...], "_note"? }`) instead of a bare array.
- **New shared helpers**: `resolve_client_id` collapses the copy-pasted client-slug→id resolution blocks across the knowledge handlers; `truncate_str` is the single char-boundary-safe truncation used by both the compact handoff-body and knowledge-snippet paths.
- **Dead code removed**: `list_stale_knowledge` (zero callers) and `deserialize_flexible_i32` (zero references).
...
- **`add_knowledge` embeds once, not twice.** The duplicate-detection check and the post-insert vector store were each calling the embedding API on the same text — two round-trips per entry. The dup-check vector is now reused as the stored embedding (identical text via a shared `embeddings::prepare_text` helper), halving embedding calls on the common (non-`force`) path. Side fix: the dup-check previously embedded the *untruncated* title+content while the stored vector used the 6,000-char-truncated form, so long entries were compared against a different vector than they were indexed with — both now use the truncated text.
- **Reads no longer ship the embedding column.** `Knowledge`/`Handoff` have no `embedding` field, but `SELECT *` / `SELECT k.*` dragged the 768-dim vector (~3KB/row) across the wire for `FromRow` to discard. Read paths now use explicit `KNOWLEDGE_COLS` / `HANDOFF_COLS` column lists (FTS, vector, hybrid-RRF, browse, list, backfill, reply-threading); writes keep `RETURNING *`.
- **Connection pool sized for a shared-Postgres co-tenant.** Dropped `max_connections` 20 → 5 with `min_connections(1)`, a 5s `acquire_timeout`, and idle/lifetime recycling (600s/1800s) — four agents plus a 5-min wake poll never needed 20 backends on a small VPS.
- **Release profile tuned for a smaller, faster binary**: `lto = "thin"`, `codegen-units = 1`, `strip = true`.
- **Trimmed `tokio` from `["full"]` to the seven features actually used** (`rt-multi-thread`, `macros`, `net`, `time`, `sync`, `io-std`, `io-util`), shrinking the compile-time feature surface.
