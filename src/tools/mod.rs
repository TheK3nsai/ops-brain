...
impl OpsBrain {
    ...
    async fn generate_briefing(
        ...
        let briefing_type = p.briefing_type.to_lowercase();
        let is_weekly = briefing_type == "weekly";

        // Resolve optional client
        let client = match &p.client_slug {
            Some(slug) => match crate::repo::client_repo::get_client_by_slug(&self.pool, slug).await {
                Ok(Some(c)) => Some(c),
                Ok(None) => return Ok(not_found("Client", slug)),
                Err(e) => return Ok(error_result(&format!("Database error: {e}"))),
            },
        ...
        let client_name = client.as_ref().map(|c| c.name.as_str()).unwrap_or("All Clients");

        // ── Monitoring ──
        let monitoring_data = if let Some(kuma_config) = &self.kuma_config {
        ...
        // ── Open incidents ──
        let open_incidents = crate::repo::incident_repo::list_incidents(
            &self.pool,
            client_id,
            Some("open"),
            None,
        ...
        // ── Zammad tickets ──
        let ticket_data = if let Some(zammad) = &self.zammad_config {
            if let Some(ref c) = client {
                if let Some(org_id) = c.zammad_org_id {
                    ...
                    let new_count = open_tickets
                        .iter()
                        .filter(|t| {
                            t.state.as_deref() == Some("new")
                        })
                        .count();
                    ...
                    Some(briefings::TicketSummaryData {
                        open_count: open_tickets.len(),
                        new_count,
                        by_priority,
                    })
                } else {
                ...
                // Global briefing — search across all orgs
                let open_query = "state.name:new OR state.name:open";
                let open_tickets = crate::zammad::search_tickets(zammad, open_query, 100)
                    .await
                    .unwrap_or_default();
        ...
        // ── Weekly stats (resolved incidents in last 7 days) ──
        let weekly_stats = if is_weekly {
            let resolved: Vec<Incident> = sqlx::query_as(
            ...
            let watchdog_resolved: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM incidents WHERE title LIKE $1 AND status = 'resolved' \
                 AND resolved_at >= NOW() - INTERVAL '7 days'",
            )
            .bind(&watchdog_prefix)
            .fetch_one(&self.pool)
        ...
        match monitoring_data.status.as_str() {
            "ALL_CLEAR" => md.push_str(&format!(
                "**ALL CLEAR** — {}/{} monitors up\n\n",
                monitoring_data.up, monitoring_data.total
            )),
            "DEGRADED" => {
                md.push_str(&format!(
                    "**DEGRADED** — {} DOWN out of {} monitors\n\n",
                    monitoring_data.down, monitoring_data.total
                ));
                for name in &monitoring_data.down_monitors {
        ...
        if monitoring_data.maintenance > 0 {
            md.push_str(&format!(
                "{} in maintenance\n\n",
                monitoring_data.maintenance
            ));
        }
        ...
        if open_incidents.is_empty() {
            ...
            md.push_str(&format!(
                "**{} open incident(s)**\n\n",
                incident_data.open_total
            ));
            // Sort severity display
            for sev in &["critical", "high", "medium", "low"] {
                if let Some(&count) = incident_data.by_severity.get(*sev) {
                    md.push_str(&format!("- {sev}: {count}\n"));
                }
            }
            if let Some(ref oldest) = incident_data.oldest_open {
                md.push_str(&format!("\nOldest open since: {oldest}\n"));
            }
            md.push('\n');
            if incident_data.watchdog_open > 0 {
                md.push_str(&format!(
                    "{} auto-detected (watchdog) incident(s) still open\n\n",
                    incident_data.watchdog_open
                ));
            }
        }
        ...
        if handoff_data.pending_count == 0 {
            md.push_str("No pending handoffs.\n\n");
        } else {
            md.push_str(&format!(
                "**{} pending handoff(s)**\n\n",
                handoff_data.pending_count
            ));
            for title in &handoff_data.pending_titles {
        ...
        // Tickets
        if let Some(ref tickets) = ticket_data {
            md.push_str("## Tickets\n\n");
        ...
        // Weekly stats
        if let Some(ref stats) = weekly_stats {
        ...
        // Store the briefing
        let briefing = match crate::repo::briefing_repo::insert_briefing(
            &self.pool,
            &briefing_type,
            client_id,
            &md,
        ...
        {
            Ok(b) => b,
            Err(e) => return Ok(error_result(&format!("Failed to store briefing: {e}"))),
        };

        let result = briefings::BriefingData {
            briefing_type: briefing_type.clone(),
            client: p.client_slug.clone(),
