...
- [x] **Verify fresh `docker compose up` works** with only `.env.example` values — 3 bugs fixed (PG18 volume path, empty ZAMMAD_DEFAULT_OWNER_ID, stdio transport default)
- [x] **Version bump** — Cargo.toml 0.1.0 → 1.0.0

## Pre-Release: Documentation

- [x] **README.md rewrite for public audience** — quick start (docker compose), feature highlights, tool tables condensed, roadmap simplified, internal phase references removed, clone URL fixed
...
2. ~~**Verify `docker compose up`**~~ — DONE. 3 bugs fixed, CC-Cloud verified healthy.
